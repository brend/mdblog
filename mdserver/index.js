...
app.get(`${config.API_PATH}/post`, (req, res) => {
...
app.get(`${config.API_PATH}/post/:id`, (req, res) => {
...
app.post(`${config.API_PATH}/post`, (req, res) => {
    if (!req.body) {
...
app.patch(`${config.API_PATH}/post/:id`, (req, res) => {
    if (!req.body) {
        console.error('request body missing');
        res.status(400).send(failure('bad request'));
        return;
